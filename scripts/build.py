#!/usr/bin/env python3
"""Build the static website and full-detail PDF from cv.json."""
import json
...
def project(p):
    ...
    return f'<article class="project"><div class="project-heading"><p class="client">{e(p["client"])}</p><h4>{e(p["title"])}</h4><p class="scope">{e(p["scope"])}</p><p class="project-date">{e(p["dates"])}</p>{note}</div>{bullets(p["bullets"])}</article>'
projects=d['pwc']['projects']
head=f'''<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Paola Carvajal | Operations Strategy &amp; Transformation Leader</title>
...
<link rel="icon" href="assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="assets/style.css?v=20260918-full"></head>
...
<main id="main"><section class="hero wrap" aria-labelledby="name"><p class="eyebrow">Professional profile · September 2026</p><h1 id="name">Paola Carvajal <span class="inline-degrees">MBA, MIB</span></h1><h2 class="positioning">{e(d['headline'])}</h2><p class="specialties">{e(d['specialties'])}</p><div class="contact-line"><span>{e(d['location'])}</span><a href="tel:+522224807474">{e(d['phone'])}</a><a href="mailto:{d['email']}">{d['email']}</a></div><div class="actions"><a class="button primary" href="PCS%20CV.pdf?v=20260918-full" download="Paola_Carvajal_Full_CV_2026.pdf">Download full CV <span aria-hidden="true">↗</span></a><a class="button" href="{d['linkedin']}" target="_blank" rel="noopener noreferrer">LinkedIn <span aria-hidden="true">↗</span></a><a class="button" href="#experience">Explore experience ↓</a></div></section>
<section class="profile wrap" aria-labelledby="profile-heading"><h2 id="profile-heading">Executive profile</h2><div class="profile-copy"><p>{e(d['summary'])}</p><p>{e(d['summary_detail'])}</p><p>{e(d['summary_earlier'])}</p></div></section>
<section class="impact" aria-label="Program leadership at PepsiCo"><div class="wrap"><p class="eyebrow">Program leadership · PepsiCo</p><div class="metric-grid">'''
html=head+''.join(f'<div class="metric"><strong>{m["value"]}</strong><b>{m["label"]}</b><small>{e(m["context"])}</small></div>' for m in d['program_metrics'])+'</div></div></section>'
html+='<section class="career-impact wrap" aria-label="Earlier career impact"><h2>Earlier career impact</h2><div class="metric-grid">'+''.join(f'<div class="metric"><strong>{m["value"]}</strong><b>{m["label"]}</b><small>{e(m["context"])}</small></div>' for m in d['metrics'])+'</div></section>'
html+='<section class="section wrap" id="experience">'+section_heading('01 / EXPERIENCE','Program, project & transformation leadership','Consulting programs, operational change and measurable business outcomes.')
html+=f'<div class="employer"><div><h3>{e(d["pwc"]["company"])}</h3><p>{e(d["pwc"]["role"])}</p></div><span class="date">{e(d["pwc"]["dates"])}</span></div><div class="full-projects">'+''.join(project(x) for x in projects)+'</div><aside class="recognition"><h3>Recognition &amp; people leadership</h3>'+bullets(d['recognition'])+'</aside></section>'
html+='<section class="section wrap" aria-labelledby="earlier-heading"><div class="section-heading"><h2 id="earlier-heading"><span class="section-number">02 / EARLIER LEADERSHIP</span>Country management & operational leadership</h2><p>Country management, commercial growth and end-to-end supply chains.</p></div>'
for x in d['earlier']:
...
html+='</section><section class="section wrap" id="expertise">'+section_heading('03 / EXPERTISE','Core expertise')+'<div class="expertise-grid">'
...
html+='<section class="section wrap" id="credentials">'+section_heading('04 / CREDENTIALS','Education, certification & training')+'<div class="education-grid"><div>'
...
for x in d['training']:
    html+=f'<article class="credential"><span class="tag">{e(x["type"])}</span><h4>{e(x["title"])}</h4><p>{e(x["detail"])}</p>'+ ('<p>Certificate ID: '+e(x['id'])+'</p>' if x.get('id') else '')+'</article>'
...
W,H=A4
story=[P(d['name'].upper(),'name'),P(d['headline'],'headline'),P(d['specialties'],'meta'),P('MBA, MIB | '+d['location']+' | '+d['phone']+' | '+d['email'],'contact')]
story += [Paragraph(f'<link href="{d["linkedin"]}" color="#4A6D6A">linkedin.com/in/paola-carvajal-senior-manager</link>',styles['contact']),P(d['summary']),P(d['summary_detail']),P(d['summary_earlier'])]
...
story+=proj(projects[0])
story+=[PageBreak(),sec('PwC engagements | continued')]
for p in projects[1:5]: story+=proj(p)
story+=[PageBreak(),sec('PwC engagements | continued')]
for p in projects[5:]: story+=proj(p)
story+=[sec('Earlier leadership')]
for x in d['earlier'][:3]: story+=early(x)
...
for x in d['training']: story.append(P(x['title']+' | '+x['type']+' | '+x['detail']+(' | Certificate ID: '+x['id'] if x.get('id') else '')))
...
pdf=SimpleDocTemplate(str(ROOT/'PCS CV.pdf'),pagesize=A4,rightMargin=42,leftMargin=42,topMargin=55,bottomMargin=44,title='Paola Carvajal | Operations Strategy & Transformation Leader',author='Paola Carvajal',subject='Professional CV - September 2026')
